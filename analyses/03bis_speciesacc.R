# Get species accumulation curves
...
n_cores <- 1 # 8
# compute for all
system.time({sa_all <- spe_acc_mclapply(long$species, long$observationID, 
                                        nrep = 50, n_cores = n_cores)})
#582.589 sec, nrep=50
...
for (i in sort(unique(long$"dbID"))){
  ...
  if (nodup(subi$observationID)>10000){
    sa_db[[i]] <- spe_acc_mclapply(subi$species, subi$observationID, 
                                   nrep = 50, n_cores = n_cores)
  }
}
